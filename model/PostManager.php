...
class PostManager extends Manager {
    ...
    public function getPosts() {
        $db=this->dbConnect();
        $articles = $db->prepare('SELECT id, title, content, DATE_FORMAT(creation_date, \'%d/%m/%Y à %Hh%imin%ss\') AS creation_date_fr FROM t_article ORDER BY creation_date DESC LIMIT 0, 5');
